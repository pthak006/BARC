...
class GRPOScriptArguments(trl.ScriptArguments):
    ...
    # Inside the GRPOScriptArguments class definition in BARC/grpo/configs.py

    reward_funcs: list[str] = field(
        # Updated default to match the planned rewards in your YAML
        default_factory=lambda: ["accuracy", "format", "nll_bit_length"], 
        metadata={
            # Updated help string to reflect BARC's relevant/planned rewards
            "help": (
                "List of reward functions to use. Possible values in this project: "
                "'accuracy' (ARC grid execution), "
                "'format' (Custom format check), "
                "'nll_bit_length' (Planned NLL score), "
                "'tag_count' (Checks for specific tags), "
                # Add keys for other rewards from open-r1 rewards.py if you plan to keep/use them
                # e.g., "'length', 'cosine', 'repetition_penalty', etc."
            )
        },
    )
    ...
    reward_weights: Optional[Dict[str, float]] = field(
        default=None,
        metadata={"help": "Optional dictionary mapping reward function names to their weights."}
    )